import { getTestClient } from 'shared-cosmos-client'
import { osmosisApi } from 'api-imperator'
import { getPoolTvls } from '../src/service/pools'
import { Pool, StableSwapPool } from '../src/model/pools.types'

main()
async function main() {
    const osmosis = getTestClient('osmosis')
    ...
    withTvl.slice(0, 20).forEach((a) => console.log(a.id, parseInt(a.liquidityUsd!)))
    ...
    await Promise.all(
        first.map(async (pool) => {
            ...
            const impPoolResponse = await osmosisApi.pools.poolById(parseInt(pool.id))
